...
type MotionHighlightProps<T extends string> = JSX.HTMLAttributes<HTMLDivElement> &
  (
...
function MotionHighlight<T extends string>({ ref, ...props }: MotionHighlightProps<T>) {
  ...
  const render = React.useCallback(
    (children: React.ReactNode) => {
      if (mode === "parent") {
        return (
          <div class={cn("relative", (props as ParentModeMotionHighlightProps)?.containerClassName)} data-slot="motion-highlight-container" ref={localRef}>
        ...
        ? controlledItems
          ...
          : render(
              React.Children.map(children, (child, index) => (
                <MotionHighlightItem class={props?.itemsClassName} key={index}>
                  {child}
                </MotionHighlightItem>
              )),
            )
...
function MotionHighlightItem({
        ...
        <>
          <AnimatePresence initial={false}>
            {isActive && !isDisabled && (
              <motion.div
                animate={{ opacity: 1 }}
                class={cn("absolute inset-0 bg-muted z-0", contextClassName, activeClassName)}
                data-slot="motion-highlight"
                exit={{
                  opacity: 0,
                  transition: {
          ...
          <div class={cn("relative z-[1]", className)} data-slot="motion-highlight-item" {...dataAttributes}>
            {children}
          </div>
        </>,
      );
  ...
  return enabled ? (
    <div
      class={cn(mode === "children" && "relative", className)}
      data-slot="motion-highlight-item-container"
      key={childValue}
      ref={localRef}
      {...dataAttributes}
