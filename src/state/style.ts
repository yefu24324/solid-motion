...
export const style = {
  get: (element: Element, name: string): string | undefined => {
    let value = isCssVar(name) ? (element as HTMLElement).style.getPropertyValue(name) : getComputedStyle(element)[name as MotionStyleKey];
    if (!value && value !== "0") {
...
export function createStyles(keyframes?: MotionStyle | DOMKeyframesDefinition): any {
  ...
  for (let key in keyframes as any) {
    ...
    if (definition) {
      // @ts-ignore
      initialKeyframe = isNumber(value) ? definition.toDefaultUnit?.(value as number) : value;
