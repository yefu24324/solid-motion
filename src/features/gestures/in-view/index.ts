...
export class InViewGesture extends Feature {
  ...
  update(): void {
    ...
    const hasOptionsChanged = ["amount", "margin", "root"].some(hasViewportOptionChanged(props as any, prevProps as any));
...
function hasViewportOptionChanged({ inViewOptions = {} }: Options, { inViewOptions: prevViewport = {} }: Options = {}) {
  return (name: keyof typeof inViewOptions) => inViewOptions[name] !== prevViewport[name];
}
